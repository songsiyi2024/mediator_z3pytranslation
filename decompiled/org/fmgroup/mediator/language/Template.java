...
DeclarationCollection<Declaration> {
    ...
    public Template fromContext(ParserRuleContext context, RawElement parent) throws ValidationException {
        ...
        for (MediatorLangParser.LocalVariableDefContext paramContext : ((MediatorLangParser.TemplateContext)context).localVariableDef()) {
            ...
            this.addDeclaration(vardecl);
        }
        return this;
    }

    public String toString() {
        return this.params.stream().map(param -> param.toString()).collect(Collectors.joining(", "));
    }
    ...
    public Template copy(RawElement parent) throws ValidationException {
        return new Template().setParent(parent).setDeclarationList((List)this.params);
    }
    ...
    public int size() {
        return this.getDeclarationList().stream().map(declaration -> declaration.getIdentifiers().size()).mapToInt(Integer::valueOf).sum();
    }
}
