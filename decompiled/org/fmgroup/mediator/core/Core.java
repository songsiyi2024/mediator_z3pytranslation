    ...
    public static void main(String[] args) throws ValidationException, SimulatorException {
        if (args.length == 0) {
            ...
            for (Class<Plugin> plugin : plugins) {
                try {
                    Plugin inst = plugin.newInstance();
                    if (!inst.isBuiltIn()) {
                        str_plugins = str_plugins + String.format("> %s ver. %s (%s)\n", inst.getName(), inst.getVersion(), inst.getDescription());
                    }
                    if (!(inst instanceof Command)) continue;
                    str_commands = str_commands + String.format("- %10s (from plugin %s)\n", ((Command)inst).getCommandName(), inst.isBuiltIn() ? "<built-in>" : inst.getName());
                }
            ...
            for (Class<Command> command : commands) {
                ...
                try {
                    Command cmdinst = command.newInstance();
                    if (!cmdinst.getCommandName().equals(args[0])) continue;
