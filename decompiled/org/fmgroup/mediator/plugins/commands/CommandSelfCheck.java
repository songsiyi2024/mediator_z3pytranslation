...
implements Command {
    ...
    public void run(Namespace args) throws FileNotFoundException {
        int counter = 0;
        for (Class<Plugin> plugin : UtilClass.getPlugins()) {
            try {
                Plugin p = plugin.newInstance();
                if (!(p instanceof Generator)) continue;
                for (String libraryname : p.requiredLibraries()) {
                    File libfolder = Paths.get(ToolInfo.getSystemLibraryPath(), libraryname).toFile();
        ...
        if (counter == 0) {
            ...
            System.err.println(String.format("Oops, %d errors found in self-checking.", counter));
        }
    }
